...
|---|---|
| `smelt_tools_and_armor.js` | Every tool, armor & glove piece, detected by scanning all crafting recipes at load (324 recipes on a loaded ATM10 world). Smithing-only gear (vanilla netherite, Aether's netherite gloves) is listed by hand since it has no crafting recipe. |
| `smelt_tools_and_armor.js` (foundry section) | Misc items with no tool slot: cauldrons, buckets, anvils, shears, minecarts, crossbows, golden apples/carrots, bells, lodestone, copper horse armor (all oxidation/waxed states), the full quartz block family, and every metal lantern (vanilla, Supplementaries, EverythingCopper, Bibliocraft, Macaw's Lights, Chipped). |
